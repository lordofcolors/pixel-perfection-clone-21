...
export function IndividualLearnerAnalytics({ 
  ...
  const generateLearnerData = (learnerName: string, index: number): LearnerData => {
    // Create varied but consistent data based on name
    const nameHash = learnerName.split('').reduce((a, b) => a + b.charCodeAt(0), 0);
    const baseSessions = 35 + (nameHash % 20); // 35-54 sessions
    const baseEngagement = 75 + (nameHash % 20); // 75-94%
    const baseFlagged = nameHash % 3; // 0-2 flagged content
    
    const themes: LessonTheme[] = [
      {
        category: "Pet Training",
        topics: ["Basic commands", "Leash training", "House training", "Social behavior"],
        sessionCount: 15 + (nameHash % 8),
        avgDuration: "15m 45s",
        engagementRate: 88 + (nameHash % 10),
        flaggedSessions: nameHash % 2,
        sessions: [
          {
            id: `session-${learnerName}-1`,
            title: "Basic Dog Walking",
            duration: "15m 30s",
            timestamp: `${index + 1}h ago`,
            engagementScore: 90 + (nameHash % 10),
            keyTopics: ["Leash training", "Safety", "Commands"],
            flagged: false
          },
          {
            id: `session-${learnerName}-7`,
            title: "Teaching Sit Command",
            duration: "12m 15s",
            timestamp: `${index + 1}d ago`,
            engagementScore: 85 + (nameHash % 10),
            keyTopics: ["Basic commands", "Rewards", "Patience"],
            flagged: nameHash % 4 === 0
          },
          {
            id: `session-${learnerName}-8`,
            title: "House Training Tips",
            duration: "18m 45s",
            timestamp: `${index + 2}d ago`,
            engagementScore: 80 + (nameHash % 15),
            keyTopics: ["House training", "Schedule", "Consistency"],
            flagged: nameHash % 5 === 0
          }
        ]
      },
      {
        ...
        sessionCount: 12 + (nameHash % 6),
        avgDuration: "18m 20s",
        engagementRate: 75 + (nameHash % 15),
        flaggedSessions: 0,
        sessions: [
          {
            id: `session-${learnerName}-3`,
            title: "Algebra Problem Solving",
            duration: "20m 15s",
            timestamp: `${index + 2}d ago`,
            engagementScore: 70 + (nameHash % 20),
            keyTopics: ["Equations", "Variables", "Problem solving"],
            flagged: nameHash % 6 === 0
          },
          {
            id: `session-${learnerName}-9`,
            title: "Math Homework Help",
            duration: "16m 30s",
            timestamp: `${index + 3}d ago`,
            engagementScore: 75 + (nameHash % 15),
      ...
      },
      {
        ...
        sessionCount: 8 + (nameHash % 6),
        avgDuration: "22m 10s",
        engagementRate: 80 + (nameHash % 15),
        flaggedSessions: 0,
        sessions: [
          {
            id: `session-${learnerName}-4`,
          ...
          {
            id: `session-${learnerName}-10`,
            title: "Communication Practice",
            duration: "25m 20s",
            timestamp: `${index + 4}d ago`,
            engagementScore: 80 + (nameHash % 12),
    ...
    const recentSessions = themes.flatMap(theme => theme.sessions).slice(0, 3);

    return {
      name: learnerName,
      totalSessions: baseSessions + index * 2,
      totalTime: `${Math.floor((baseSessions + index * 2) * 0.25)}h ${15 + (nameHash % 45)}m`,
      avgEngagement: Math.min(baseEngagement + index * 2, 98),
      completionRate: 70 + (nameHash % 25) + index * 2,
      flaggedContent: baseFlagged,
      lastActive: index === 0 ? "2h ago" : `${index}d ago`,
      themes,
      recentSessions
    };
  };
  ...
  const learnerData: Record<string, LearnerData> = learners.reduce((acc, learner, index) => {
    acc[learner.name] = generateLearnerData(learner.name, index);
    return acc;
  }, {} as Record<string, LearnerData>);
