      ...
      <main className="w-screen min-h-screen relative overflow-auto font-literata bg-background max-md:p-5 max-sm:p-4">
        ...
        <form onSubmit={handleDoneClick} noValidate>
          ...
          <fieldset className="flex gap-4 absolute -translate-x-2/4 left-2/4 top-[585px] max-md:flex-col max-md:items-center max-md:gap-5 max-md:top-[480px] max-sm:top-[400px]">
            ...
            {roleCards.map((card) => (
              <div key={card.role} className="relative">
                <RoleCard
                  role={card.role}
                  title={card.title}
                ...
                <Tooltip>
                  ...
                  <TooltipContent side="top" className="max-w-[280px] p-3 font-literata">
                    <p className="font-semibold mb-1">{roleInfo[card.role].title}</p>
                    <p className="text-sm text-muted-foreground">{roleInfo[card.role].description}</p>
                  </TooltipContent>
                </Tooltip>
              </div>
            ))}
